...
node.prototype.ExportToLine = function () {
    var txt = '';
    var iOpto = this.iOpt.toString();
    var nLen = iOpto.length;
    switch (nLen) {
        case 1:
            txt = txt + iOpto+'    ' + this.iType + FormatNumberByLen(this.v[1], 12, 6);
            break;
        case 2:
            txt = txt + iOpto + '   ' + this.iType + FormatNumberByLen(this.v[1], 12, 6);
            break;
...
node.prototype.IsJunc = function () {
    var ind = this.iType;
...
design.prototype.ConnLogsExport = function () {
    // design schema (juns) to text
    var txt = '';
    var i, nod = false;
    for (i = 0; i <= this.nJuncs; i++) {
        nod = !nod;
        if (this.juncs[i].IsNode() || this.juncs[i].IsJunc()) {
            if (nod) {
                if (!this.juncs[i].IsNode()) {
                    txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
                    txt = txt + ':  Can be type 1 only, but type ' + this.juncs[i].iType + ' found <br/>';
                }
            } else {
                if (!this.juncs[i].IsJunc()) {
                    txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
                    txt = txt + 'Element #' + i + ':  Can be type 0,2,3, but type ' + this.juncs[i].iType + ' found <br/>';
                }

            }
        } else {
            txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
            txt = txt + ':  Wrong element type ' + this.juncs[i].iType + '<br/>';
        }
    }
    if (txt === '') {
        return null;
...
design.prototype.SymLogsExport = function () {
    // design schema (juns) to text
    var txt = '';
    var nCount = this.nJuncs;
    var i, nMid = parseInt(nCount / 2);
    for (i = 0; i <= nCount; i++) {
       if (this.iSymZ === 1 && i <= nMid) {
            if (this.juncs[i].iType !== this.juncs[nCount - i].iType) {
                txt = txt + 'Lines  | <font style="color:red"> ' + i + '</font> and ' + (this.nJuncs - i) + '</font> do not match';
                txt = txt + ': Z-symmetry violation <br/>';
            }
        }
        if (!IsSymNode(this.iSymX, 0, this.juncs[i])) {
            txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
            txt = txt + ': X-symmetry violation <br/>';
        }
        if (!IsSymNode(this.iSymY, 1, this.juncs[i])) {
            txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
            txt = txt + ': Y-symmetry violation <br/>';
        }
    }
    if (txt === '') {
        return null;
...
design.prototype.DimLogsExport = function () {
    // design schema (juns) to text
    var txt = '';
    var i;
    for (i = 0; i <= this.nJuncs; i++) {
        switch (this.juncs[i].iType) {
            case 0:
                if (i > 0 && i < this.nJuncs) {
                    if (!IsConnect(this.juncs[i - 1], this.juncs[i + 1])) {
                        txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
                        txt = txt + ': Size mismatch with adjacent nodes<br/>';
                    }
                } else {
                    txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
                    txt = txt + ': Cannot start/end with type ' + this.juncs[i].iType + '<br\>';
                }
                break;
            case 1:
                break;
            case 2:
                if (i > 0 && i < this.nJuncs) {
                    if (!IsNodeLE(this.juncs[i - 1], this.juncs[i])) {
                        txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
                        txt = txt + ': Size mismatch with previous node <br/>';
                    }
                    if (!IsNodeLE(this.juncs[i + 1], this.juncs[i])) {
                        txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
                        txt = txt + ': Size mismatch with next node<br\>';
                    }
                } else {
                    txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
                    txt = txt + ': Cannot start/end with type ' + this.juncs[i].iType + '<br\>';
            ...
            case 3:
                if (i > 0 && i < this.nJuncs) {
                    if (!IsNodeLE(this.juncs[i], this.juncs[i - 1])) {
                        txt = txt + 'Line ' + i + ' | <font style="color:red">' + this.juncs[i].ExportToLine() + '</font>';
                        txt = txt + ': Size mismatch with previous node <br/>';
                    }
                    if (!IsNodeLE(this.juncs[i], this.juncs[i + 1])) {
                ...
                break;
            default:
                txt = txt + 'Wrong node/junction index ' + this.juncs[i].iType + ' at element #' + i + '\r\n';
        }
    }
    if (txt === '') {
        return null;
...
design.prototype.IsSymZ = function () {
    var i, Count = this.nJuncs;
    if (Count < 1) { return false; }
    for (i = 0; i <= Count; i++) {
        if (this.juncs[i].iType !== this.juncs[Count - i].iType) {
            return false;
        }
    }
    return true;
};
design.prototype.FixSymZ = function () {
    var i, x, Count = this.nJuncs;
    var iCount = parseInt(Count / 2);
    if (Count < 1) { return;}
    for (i = 0; i <= iCount; i++) {
        if (this.juncs[i].iType !== 0) {
            for (iv = 0; iv <= 5; iv++) {
                x = 0.5 * (this.juncs[i].v[iv] + this.juncs[Count - i].v[iv]);
                this.juncs[i].v[iv] = x; this.juncs[Count - i].v[iv] = x;
            }
        }
    }
};
design.prototype.FixJuncDims = function () {
    var i, Count = this.nJuncs;
    for (i = 0; i <= Count; i++) {
        if (!this.juncs[i].IsNode()) {
            switch (this.juncs[i].iType) {
                case 2:
                    if (this.juncs[i - 1].v[2] < this.juncs[i].v[2]) {
                        this.juncs[i].v[2] = this.juncs[i - 1].v[2];
                    }
                    if (this.juncs[i + 1].v[2] < this.juncs[i].v[2]) {
                        this.juncs[i].v[2] = this.juncs[i + 1].v[2];
                    }
                    if (this.juncs[i - 1].v[3] < this.juncs[i].v[3]) { this.juncs[i].v[3] = this.juncs[i - 1].v[3]; }

                    if (this.juncs[i + 1].v[3] < this.juncs[i].v[3]) { this.juncs[i].v[3] = this.juncs[i + 1].v[3]; }

                    if (this.juncs[i - 1].v[4] > this.juncs[i].v[4]) { this.juncs[i].v[4] = this.juncs[i - 1].v[4]; }

                    if (this.juncs[i + 1].v[4] > this.juncs[i].v[4]) { this.juncs[i].v[4] = this.juncs[i + 1].v[4]; }
                    if (this.juncs[i - 1].v[5] > this.juncs[i].v[5]) { this.juncs[i].v[5] = this.juncs[i - 1].v[5]; }
                    if (this.juncs[i + 1].v[5] > this.juncs[i].v[5]) { this.juncs[i].v[5] = this.juncs[i + 1].v[5]; }
                    break;
                case 3:
                    if (this.juncs[i - 1].v[2] > this.juncs[i].v[2]) { this.juncs[i].v[2] = this.juncs[i - 1].v[2]; }
                    if (this.juncs[i + 1].v[2] > this.juncs[i].v[2]) { this.juncs[i].v[2] = this.juncs[i + 1].v[2]; }
                    if (this.juncs[i - 1].v[3] > this.juncs[i].v[3]) { this.juncs[i].v[3] = this.juncs[i - 1].v[3]; }
                    if (this.juncs[i + 1].v[3] > this.juncs[i].v[3]) { this.juncs[i].v[3] = this.juncs[i + 1].v[3]; }
                    if (this.juncs[i - 1].v[4] < this.juncs[i].v[4]) { this.juncs[i].v[4] = this.juncs[i - 1].v[4]; }
...
design.prototype.FixNodesDims = function () {
    var i, Count, Xm0, Xm1, Ym0, Ym1;
    Count = this.nJuncs;
    for (i = 1; i <= Count - 1; i++) {
        if (this.juncs[i].IsNode()) {
            Xm0 = this.juncs[i].v[2]
            if (this.juncs[i - 1].iType === 2 && this.juncs[i - 1].v[2] > Xm0) { Xm0 = this.juncs[i - 1].v[2]; }
            if (this.juncs[i + 1].iType === 2 && this.juncs[i + 1].v[2] > Xm0) { Xm0 = this.juncs[i + 1].v[2]; }
            if (this.juncs[i - 1].iType === 3 && this.juncs[i - 1].v[2] < Xm0) { Xm0 = this.juncs[i - 1].v[2]; }
            if (this.juncs[i + 1].iType === 3 && this.juncs[i + 1].v[2] < Xm0) { Xm0 = this.juncs[i + 1].v[2]; }
            this.juncs[i].v[2] = Xm0
            Xm1 = this.juncs[i].v[4];
            if (this.juncs[i - 1].iType === 2 && this.juncs[i - 1].v[4] < Xm1) { Xm1 = this.juncs[i - 1].v[4]; }
            if (this.juncs[i + 1].iType === 2 && this.juncs[i + 1].v[4] < Xm1) { Xm1 = this.juncs[i + 1].v[4]; }
            if (this.juncs[i - 1].iType === 3 && this.juncs[i - 1].v[4] > Xm1) { Xm1 = this.juncs[i - 1].v[4]; }
            if (this.juncs[i + 1].iType === 3 && this.juncs[i + 1].v[4] > Xm1) { Xm1 = this.juncs[i + 1].v[4]; }
            this.juncs[i].v[4] = Xm1
            Ym0 = this.juncs[i].v[3];
            if (this.juncs[i - 1].iType === 2 && this.juncs[i - 1].v[3] > Ym0) { Ym0 = this.juncs[i - 1].v[3]; }
            if (this.juncs[i + 1].iType === 2 && this.juncs[i + 1].v[3] > Ym0) { Ym0 = this.juncs[i + 1].v[3]; }
            if (this.juncs[i - 1].iType === 3 && this.juncs[i - 1].v[3] < Ym0) { Ym0 = this.juncs[i - 1].v[3]; }
            if (this.juncs[i + 1].iType === 3 && this.juncs[i + 1].v[3] < Ym0) { Ym0 = this.juncs[i + 1].v[3]; }
            this.juncs[i].v[3] = Ym0
            Ym1 = this.juncs[i].v[5];
            if (this.juncs[i - 1].iType === 2 && this.juncs[i - 1].v[5] < Ym1) { Ym1 = this.juncs[i - 1].v[5]; }
            if (this.juncs[i + 1].iType === 2 && this.juncs[i + 1].v[5] < Ym1) { Ym1 = this.juncs[i + 1].v[5]; }
            if (this.juncs[i - 1].iType === 3 && this.juncs[i - 1].v[5] > Ym1) { Ym1 = this.juncs[i - 1].v[5]; }
            if (this.juncs[i + 1].iType === 3 && this.juncs[i + 1].v[5] > Ym1) { Ym1 = this.juncs[i + 1].v[5]; }
            this.juncs[i].v[5] = Ym1;
        }
    }
};
...
// GT GE EQ LT GT
function IsSymNode(iSym, iVar, J) {
    // iSymX, iVar=0; iSymY, iVar=1
    var delta = 1E-7;
    if (iSym === 0 || J.iType === 0) { return true;}
    if (iSym === 1 || iSym === -1) {
        if (Math.abs(J.v[iVar + 2] + J.v[iVar + 4]) < delta) {
            return true;
        }
    }
    return false;
...
function IsNodeEQ(J0, J1) {
    //X0=v[1] Y0=v[2] X1=v[3] Y1=v[4]
    if (J0.v[2] === J1.v[2] && J0.v[3] === J1.v[3] && J0.v[4] === J1.v[4] && J0.v[5] === J1.v[5]) {
        return true;
    } else {
        return false;
    }
}
function IsNodeNE(J0, J1) {
    if (IsNodeEQ(J0, J1)) {
        return false;
    } else {
        return true;
...
function IsNodeGT(J0, J1) {
    //J0 > J1
    return IsNodeLT(J1, J0);
}
function IsNodeLE(J0, J1) {
    if (IsNodeLT(J0, J1) || IsNodeEQ(J0, J1)) {
        return true;
    } else {
        return false;
    }
}
function IsNodeGE(J0, J1) {
    if (IsNodeLT(J1, J0) || IsNodeEQ(J1, J0)) {
        return true;
    } else {
        return false;
    }
}
function IsConnect(J0, J1) {
    if (IsNodeLE(J0, J1) || IsNodeLE(J1, J0)) {
        return true;
    } else {
        return false;
    }
}
function NoConnect(J0, J1) {
    return !IsConnect(J0, J1);
}
function FixCav(J0, J, J1) {
    var JJ = new node;
    fix = J;
    if (JJ.v[1] > J0.v[1] || JJ.v[1] > J1.v[1]) {
        JJ.v[1] = Math.min(J0.v[1], J1.v[1]);
    }
    if (JJ.v[2] > J0.v[2] || JJ.v[2] > J1.v[2]) {
        JJ.v[2] = Math.min(J0.v[2], J1.v[2]);
    }
    if (JJ.v[3] < J0.v[3] || JJ.v[3] < J1.v[3]) {
        JJ.v[3] = Math.max(J0.v[3], J1.v[3]);
    }
    if (JJ.v[4] < J0.v[4] || JJ.v[4] < J1.v[4]) {
        JJ.v[4] = Math.max(J0.v[4], J1.v[4]);
    }
    return JJ;
}
function FixIris(J0, J, J1) {
    var JJ = new node;
    fix = J;
    if (JJ.v[1] < J0.v[1] || JJ.v[1] < J1.v[1]) {
        JJ.v[1] = Math.max(J0.v[1], J1.v[1]);
    }
    if (JJ.v[2] < J0.v[2] || JJ.v[2] < J1.v[2]) {
        JJ.v[2] = Math.max(J0.v[2], J1.v[2]);
    }
    if (JJ.v[3] > J0.v[3] || JJ.v[3] > J1.v[3]) {
        JJ.v[3] = Math.min(J0.v[3], J1.v[3]);
    }
    if (JJ.v[4] > J0.v[4] || JJ.v[4] > J1.v[4]) {
        JJ.v[4] = Math.min(J0.v[4], J1.v[4]);
    }
    return JJ;
}
...
design.prototype.CalcFreq = function () {
    ...
    var mode = this.km, nn = nSym(this.iSymX, this.kx), mm = nSym(this.iSymY, this.ky);
    var k = aox(kf(this.iUnit, this.f), this.sqrtE);
    var iGuide = -1, zo = 376.730313668;
    if (!this.isPerfect) { this.Rs = Rsurf(this.f, this.sig, 1)/zo; } else { this.Rs = 0; }
    //HUJ
    //alert(this.Rs);
    //return;
    //END HUJ
    ...
    for (i = 0; i <= this.nJuncs; i++) {
        ...
        switch (this.juncs[i].iType) {
            case 3:
                d = this.juncs[i].v[1];
                //RecIrisC(k, rm, a0, b0, W0, o0, A0, a, b, d, a1, b1, o1, W1, A1)
                if (this.isPerfect) {
                    x = this.juncs[i].v[0];
                    iGuide = this.juncs[i - 1].iGuide;
                    Se = RecIris(k, Guides[iGuide], Apers[iAper], x, d, Guides[iGuide + 1], Apers[iAper + 1]);
                } else {
                    a0 = this.juncs[i - 1].v[4] - this.juncs[i - 1].v[2];
                    b0 = this.juncs[i - 1].v[5] - this.juncs[i - 1].v[3];
                    a = this.juncs[i].v[4] - this.juncs[i].v[2];
                    b = this.juncs[i].v[5] - this.juncs[i].v[3];
                    a1 = this.juncs[i + 1].v[4] - this.juncs[i + 1].v[2];
                    b1 = this.juncs[i + 1].v[5] - this.juncs[i + 1].v[3];
                    iGuide = this.juncs[i - 1].iGuide;
                    Se = RecIrisC(k, this.Rs, a0, b0, oApers[iAper], Apers[iAper], a, b, d, a1, b1, oApers[iAper + 1], Apers[iAper + 1]);
                }
            ...
            case 2:
                ...
                var Y0, Y1, Ye;
                if (this.isPerfect) {
                    Y0 = yw(mode, k, beta(k, this.juncs[i - 1].v[0]));
                    Y1 = yw(mode, k, beta(k, this.juncs[i + 1].v[0]));
                    Ye = RecCav(k, a, b, d, Apers[iAper], Guides[iGuide], Apers[iAper + 1]);
                } else {
                    a0 = this.juncs[i - 1].v[4] - this.juncs[i - 1].v[2];
                    b0 = this.juncs[i - 1].v[5] - this.juncs[i - 1].v[3];
                    a1 = this.juncs[i + 1].v[4] - this.juncs[i + 1].v[2];
                    b1 = this.juncs[i + 1].v[5] - this.juncs[i + 1].v[3];
                    Y0 = yw(mode, k, betC(k, this.Rs, a0, b0, mode, nn, mm));
                    Y1 = yw(mode, k, betC(k, this.Rs, a1, b1, mode, nn, mm));
                    Ye = RecCavC(k, this.Rs, a, b, d, oApers[iAper], Apers[iAper], Guides[iGuide], oApers[iAper + 1], Apers[iAper + 1]);
               }
                Se = SY2(Y0, Ye, Y1);
                break;
            case 1:
                ...
                if (this.isPerfect) {
                    ...
                    Se = Snode(betC(k, this.Rs, a, b, mode, nn, mm), d);
                }
                break;
            case 0:
                var iDir = this.juncs[i].iDir;
                var iSmall = i - iDir, iBig = i + iDir;
                a0 = this.juncs[iSmall].v[4] - this.juncs[iSmall].v[2];
                b0 = this.juncs[iSmall].v[5] - this.juncs[iSmall].v[3];
                a = this.juncs[iBig].v[4] - this.juncs[iBig].v[2];
                b = this.juncs[iBig].v[5] - this.juncs[iBig].v[3];
                iGuide = this.juncs[i + iDir].iGuide;
                var jBY, ySmall;
                //RecStepC(k, rm, a, b, o, A)
                if (this.isPerfect) {
                    jBY = RecStep(k, Guides[iGuide], Apers[iAper]);
                    ySmall = yw(mode, k, beta(k, this.juncs[iSmall].v[0]));
                } else {
                    jBY = RecStepC(k, this.Rs, a, b, oApers[iAper], Apers[iAper]);
                    ySmall = yw(mode, k, betC(k, this.Rs, a0, b0, mode, nn, mm));
                }
                if (iDir === 1) {
        ...
        if (this.juncs[i].iType === 1) {
            ...
            switch (this.juncs[i].iDir) {
                case -2:
                    ...
                    if (!this.isPerfect) { oApers[iAper] = 2*oLoss(x0, y0, x1, y1, Ao, Bo); }
                    x0 = this.juncs[i].v[2] - this.juncs[i + 1].v[2];
                    x1 = this.juncs[i].v[4] - this.juncs[i + 1].v[2];
                    y0 = this.juncs[i].v[3] - this.juncs[i + 1].v[3];
                    y1 = this.juncs[i].v[5] - this.juncs[i + 1].v[3];
                    ...
                    if (!this.isPerfect) { oApers[iAper + 1] = 2*oLoss(x0, y0, x1, y1, Ao, Bo); }
