...
function Rsurf(f, sigm, mu) {
    //Surf resistance in ohms
    // sigm in 10^7*S/m, mu - metal permeability
    // to be normalized to 376.730313668
    return 0.0198691765 * Math.sqrt(f * mu / sigm);
}
function betC(k, rm, a, b, imode, n, m) {
    //       k is complex wavenumber in medium, rm is Rm/Zo
    //        dim complex g2,j
    var pi = 3.141592653589793238;
    var en, em;
    if (n === 0) { en = 2; } else { en = 1;}
    if (m === 0) { em = 2; } else { em = 1; }
    var xn = pi / a * n, xm = pi / b * m;
    var xn2 = xn * xn, xm2 = xm * xm, x2 = xn2 + xm2;
    var v, v0, v1, ks2, g2, ck, k2;
    k2 = xin2(k);
    if (imode === 0) {
        v = 4 * rm / k[0]; //imaginary
        v0 = 1 / em / b + 1 / en / a;
        v1 = xn2 / em / b + xm2 / en / a;
        ks2 = [k2[0] - x2, k2[1] - x2 * v * v0];
        ck = [1, v * v1 / x2];
        g2 = xdy(ks2, ck);
    } else {
        var v = 4 * rm * k[0]; //imaginary
        v1 = xm2 / em / b + xn2 / en / a;
        g2 = [k2[0] - x2, k2[1] - v * v1 / x2];
    }
    return csqrt(g2);
}
